...
export function calculateTotals(data) {
  ...
  data.totals.calculated_annual_principal_payment =
    calculateAnnualPrincipalPayment(data);
...
export function calculateMonthlyTotalIncome(data) {
  const totalIncome = data.people.reduce(
    (total, person) =>
      total +
      Number(person.monthly_net_income) +
      Number(person.monthly_other_income) +
      Number(person.monthly_bonuses_dividends_income), // string → number
    0
  );
  return totalIncome;
}
...
// data.totals.calculated_annual_principal_payment
export function calculateAnnualPrincipalPayment(data, year = 0) {
...
export function calculateAnnualBudgetAvailable(data) {
  const annualBudget =
    calculateAnnualPrincipalPayment(data) +
    calculateMonthlyFinalSurplusBudget(data) * 12;
  return annualBudget;
}
